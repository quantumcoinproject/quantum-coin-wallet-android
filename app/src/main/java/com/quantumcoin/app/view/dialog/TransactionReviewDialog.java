...
public final class TransactionReviewDialog {
     ...
     */
    public static AlertDialog show(final Activity activity, final JsonViewModel vm,
        ...
        return build(activity, vm, walletAddress, s, onConfirm, onCancel);
    }
    ...
    private static AlertDialog build(final Activity activity, final JsonViewModel vm,
        ...
        prompt.setTextColor(ctx.getResources().getColor(R.color.colorCommon6));
        root.addView(prompt);
        ...
        if (spec.contractAddress != null) {
            addRow(rows, ctx, vm.lang("contract-address", "Contract address"), spec.contractAddress, true, 0,
                    spec.contractIsToken ? com.quantumcoin.app.networking.UrlBuilder.blockExplorerTokenUrl(spec.contractAddress)
                            : com.quantumcoin.app.networking.UrlBuilder.blockExplorerAccountUrl(spec.contractAddress));
        }
        if (spec.fromTokenContract != null) {
            addRow(rows, ctx, vm.lang(key(spec.fromTokenContractLabelKey, "swap-from-token-contract"),
                    ...
                    com.quantumcoin.app.networking.UrlBuilder.blockExplorerTokenUrl(spec.fromTokenContract));
        }
        if (spec.toTokenContract != null) {
            addRow(rows, ctx, vm.lang("swap-to-token-contract", "To token contract"),
                    spec.toTokenContract, true, 0, com.quantumcoin.app.networking.UrlBuilder.blockExplorerTokenUrl(spec.toTokenContract));
        }
        if (spec.fromAddress != null) {
            addRow(rows, ctx, vm.lang("from-address", "From Address"), spec.fromAddress, true, 0,
                    com.quantumcoin.app.networking.UrlBuilder.blockExplorerAccountUrl(spec.fromAddress));
        }
        if (spec.toAddress != null && !spec.toAddress.isEmpty()) {
            addRow(rows, ctx, vm.lang("to-address", "To Address"), spec.toAddress, true, 0,
                    com.quantumcoin.app.networking.UrlBuilder.blockExplorerAccountUrl(spec.toAddress));
        }
        ...
        addRow(rows, ctx, vm.lang("network", "Network"),
                spec.networkText == null ? "" : spec.networkText, false, 0xFF1B7A1B);
        ...
        int start = sb.length();
        sb.append(literal);
        sb.setSpan(new ForegroundColorSpan(0xFF1F6FEB), start, sb.length(),
                Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        sb.append(suffix);
        agreeLabel.setText(sb);
        agreeLabel.setTextSize(13);
        agreeLabel.setTextColor(ctx.getResources().getColor(R.color.colorCommon6));
        ...
        agreeField.setTextColor(ctx.getResources().getColor(R.color.colorCommon6));
        ...
        cancel.setTextColor(ctx.getResources().getColor(R.color.colorCommon7));
        ...
        ok.setAllCaps(false);
        ok.setTextColor(ctx.getResources().getColor(R.color.colorCommon7));
    ...
    private static void addRow(LinearLayout parent, Context ctx, String header, String value,
        ...
        h.setTextSize(13);
        h.setTextColor(ctx.getResources().getColor(R.color.colorCommon6));
        ...
        v.setTextSize(12);
        v.setTextColor(valueColor != 0 ? valueColor : ctx.getResources().getColor(R.color.colorCommon6));
        if (mono) v.setTypeface(Typeface.MONOSPACE);
        if (link != null) {
            com.quantumcoin.app.utils.ExplorerLinks.linkValue(v, value, link);
        } else {
